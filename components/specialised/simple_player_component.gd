class_name SimplePlayerComponent extends Component3D
...
func _ready():
	...
	GameManager.game_paused.connect(_release_mouse)
	GameManager.game_unpaused.connect(_capture_mouse)
...
func _physics_process(delta):
	...
	if crouching:
		if root.is_on_floor():
			t_pos_y = head_y - 0.85
		if root.velocity.length() > WALK_SPEED * 1.05 and enable_slide:
